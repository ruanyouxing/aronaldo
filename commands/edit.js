...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("message_id")
        .setDescription("ID của tin nhắn cần sửa")
        .setRequired(true)
    )
    .addChannelOption((option) =>
      option
        .setName("channel")
        .setDescription("Kênh chứa tin nhắn cần sửa")
    ...
    .addAttachmentOption((option) =>
      option
        .setName("cover")
        .setDescription("File ảnh bìa mới")
        .setRequired(false)
    )
  ...
  async execute(interaction) {
    ...
    if (interaction.channel.id !== COMMAND_CHANNEL_ID) {
      return interaction.editReply({
        content: `❌ Bạn chỉ có thể dùng lệnh từ kênh <#${COMMAND_CHANNEL_ID}>`,
      });
    }
    ...
    if (!outputChannel) {
      return interaction.editReply({
        content: `❌ Không tìm thấy kênh thông báo!`,
      });
    }
    ...
    try {
      ...
      if (!targetMessage) {
        return interaction.editReply({
          content: `❌ Không tìm thấy tin nhắn với ID \`${messageId}\`.`,
        });
      }

      if (targetMessage.author.id !== interaction.client.user.id) {
        return interaction.editReply({
          content: `❌ Bot chỉ có thể chỉnh sửa tin nhắn do chính nó gửi ra.`,
        });
      }
      ...
      if (oldEmbed && oldEmbed.fields) {
        ...
        // Trích xuất Archive
        const archiveField = oldEmbed.fields.find(
          (f) => f.name === "📦 Archive"
        );
        if (archiveField) {
          const arcMatch = archiveField.value.match(
            /\[Xem archive truyện\]\((.*?)\)/
          );
      ...
      const newDescription =
        interaction.options.getString("description") ?? oldEmbed?.description;
      const newCaption =
        interaction.options.getString("caption") ?? targetMessage.content;
      ...
      const archiveUrl = archiveFile
        ? archiveFile.url
        : newArchiveInput ?? oldArchive;
      ...
      for (const item of inputsToValidate) {
        if (item.url) {
          ...
          if (!result.valid) {
            return interaction.editReply({
              content: `❌ URL bạn vừa nhập không hợp lệ (${item.name}): ${result.error}`,
            });
          }
        }
      }
      ...
      if (newCoverAttachment) {
        ...
        if (targetMessage.attachments.size > 0) {
          ...
          if (isOutOfEmbed) {
            return interaction.editReply({
              content: `❌ Bạn muốn đưa ảnh ra ngoài Embed nhưng lại không cung cấp file ảnh mới. Vui lòng upload lại file ở option \`cover\`.`,
            });
          }
        }
      }
      ...
      await interaction.editReply({
        content: `✅ Đã chỉnh sửa thành công tin nhắn tại https://discord.com/channels/${serverId}/${outputChannel.id}/${messageId} !`,
      });
    } catch (error) {
      ...
      if (error.code === 10008) {
        return interaction.editReply({
          content:
            "❌ Không tìm thấy tin nhắn! Hãy chắc chắn ID tin nhắn là đúng.",
        });
      }

      await interaction.editReply({
        content: "❌ Chỉnh sửa thất bại. Có lỗi xảy ra trong quá trình xử lý.",
      });
    }
  },
};
